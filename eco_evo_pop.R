...
temp<-expand.grid(Temperature=seq(20,25,0.25)) %>%
  mutate(Equilibrium_Abundance=0)


for(r in 1:nrow(temp)){
  
  for(t in 1:tmax){
    T<-temp$Temperature[r]
    b<- (b0*w/sqrt(w^2+2*s))*exp(-(u-T)^2/(2*s+w^2)) #average rate of growth
    N[t+1] <- N[t] + N[t]*(b- d0-N[t]/K)*dt #euler approximation. This is being done in a simpler way. Euler approximation
    #is not accurate and one should switch to runge-kutta 4 or solvers for better accuracy.
  }
  
  temp$Equilibrium_Abundance[r]<- median(N,na.rm=T)
}

plot(temp$T,temp$Equilibrium_Abundance,cex=3,col="red", xlab="Temperature", ylab="Equilibrum mean abundance")

p1<-temp %>% ggplot(aes(x=(Temperature-20),y=Equilibrium_Abundance))+
  geom_point(size=4)+
  geom_line()+
  theme_classic()+
  ggtitle("No evolution of the mean phenotype")+
  geom_vline(xintercept = 1.3, linetype="dashed",col="grey2")+
  # geom_hline(yintercept = 0,linetype="dashed")+
  xlab("Mismatch in Temperature")+
